...
class LLMInferenceHelper @Inject constructor(
    ...
    override val offlineLLMStatus: StateFlow<OfflineLLMStatus> = _offlineLLMStatus.asStateFlow()

    override fun initialize() {
        if (_offlineLLMStatus.value is OfflineLLMStatus.Ready ||
            _offlineLLMStatus.value is OfflineLLMStatus.Loading
        ) return
        ...
        scope.launch {
            try {
                val result = withContext(Dispatchers.IO) {
                    ...
                    if (!modelFile.exists()) {
                        return@withContext OfflineLLMStatus.Error(context.getString(R.string.model_not_found))
                    }
                ...
                val errorMsg = context.getString(R.string.failed_init_llm, e.message)
    ...
    override fun generateResponse(prompt: String) {
        try {
            ...
            Log.e("LLMHelper", context.getString(R.string.native_engine_error, e.message))
            _partialResults.tryEmit(
                Pair(
                    context.getString(R.string.hardware_error_gpu),
                    true
                )
            )
        }
